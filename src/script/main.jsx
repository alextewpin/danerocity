	...
	render() {
		return (
			<div>
				...
				<table className='monsters'>
					<tbody>
						<tr className='monsters__title'>
							<td className='monsters__title-td' colSpan='2'>Number</td>
							<td className='monsters__title-td' colSpan='3'>Damage</td>
							<td className='monsters__title-td' colSpan='2'>Attacks</td>
						</tr>
				...
				<button className='add-monster' onClick={this.addMonster}>Add Monster</button>
				<div className='total'>Total raw damage: {this.getTotalDamage()}</div>
			</div>
		);
	}
})
...
var Monster = React.createClass({
	...
	render() {
		...
		if (this.props.canDeleteRow === true) {
			delButton =	<svg className='monster__delete' onClick={this.props.deleteMonster.bind(null, this.props.i)}>
					<path d='M10.3846154,9 L18,1.38461538 L16.6153846,-1.16020857e-14 L9,7.61538462 L1.38461538,-1.42108547e-14 L-1.42480322e-14,1.38461538 L7.61538462,9 L-1.59872116e-14,16.6153846 L1.38461538,18 L9,10.3846154 L16.6153846,18 L18,16.6153846 L10.3846154,9 L10.3846154,9 L10.3846154,9 Z'></path>
				</svg>
		}
		return (
			<tr className='monster'>
				<td className='monster__td'>
					<input className='monster__number-input' onChange={this.props.setNumber.bind(null, this.props.i)} type='range' value={this.props.number} min='1' max='8'/> 
				</td>
				<td className='monster__td monster__number'>{this.props.number}</td>
				<td className='monster__td'>
					<input className='monster__power-input' onChange={this.props.setPower.bind(null, this.props.i)} type='range' value={this.props.power} min='1' max='40'/>
				</td>
				<td className='monster__td monster__power-number'>{this.props.power}</td>
				<td className='monster__td monster__power-dice'>{this.getDices()}</td>
				<td className='monster__td'>
					<input className='monster__attacks-input' onChange={this.props.setAttacks.bind(null, this.props.i)} type='range' value={this.props.attacks} min='1' max='8'/> 
				</td>
				<td className='monster__td monster__attacks'>{this.props.attacks}</td>
				<td className='monster__td'>
					{delButton}
				</td>
			</tr>
